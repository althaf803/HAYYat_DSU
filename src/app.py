...
# ADD THIS LINE HERE
PYTHON_EXECUTABLE = "F:\\HAYYat_DSU-main\\venv\\Scripts\\python.exe"
...
def senddata_articles(general_search, twitter_search):
    success = subprocess.call([PYTHON_EXECUTABLE, 'articles.py', general_search, twitter_search])
    if success == 0:
        return load_results('news_articles.json')
    return None

def senddata_twitter_id(general_search, twitter_search):
    success = subprocess.call([PYTHON_EXECUTABLE, 'twitter_id.py', general_search, twitter_search])
    if success == 0:
        return load_results('comments_on_id.json')
    return None

def senddata_twitter_search(general_search, twitter_search):
    success = subprocess.call([PYTHON_EXECUTABLE, 'twitter_search.py', general_search, twitter_search])
    if success == 0:
        return load_results('tweets_on_search.json')
    return None

def senddata_yt(general_search, twitter_search):
    success = subprocess.call([PYTHON_EXECUTABLE, 'youtube.py', general_search, twitter_search])
    if success == 0:
